...
void locate(std::ifstream& in, string patterns){
	...
	for(ulint i=0;i<n;++i){
		...
		auto after = now();
		search += time_diff_ns(before,after);
	...
	std::cout << "locate time: " << format_time(search) << std::endl;
	std::cout << "            " << format_time(search/n) << "/pattern" << std::endl;
	std::cout << "            " << format_time(search/occ_tot) << "/occurrence" << std::endl;
